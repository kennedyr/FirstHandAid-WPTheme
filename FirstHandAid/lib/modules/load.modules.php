...
function shoestrap_include_modules() {
  ...
  // Include all modules from the child theme
  $childModules_path = new RecursiveDirectoryIterator( get_stylesheet_directory() . '/lib/modules/' );
  $childRecIterator  = new RecursiveIteratorIterator( $childModules_path );
  $childRegex        = new RegexIterator( $childRecIterator, '/\/module.php$/i' );

  foreach( $childRegex as $item ) {
    require_once $item->getPathname();
  }
}
endif;
